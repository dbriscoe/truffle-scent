...
window.TRUFFLE_SCENT_I18N?.applyLanguage?.(window.TRUFFLE_SCENT_I18N.getLanguage());

const dialog=document.querySelector('[data-product-dialog]');const dialogTitle=document.querySelector('[data-dialog-title]');const dialogCopy=document.querySelector('[data-dialog-copy]');const dialogKicker=document.querySelector('[data-dialog-kicker]');const dialogPair=document.querySelector('[data-dialog-pair], [data-dialog-best], [data-dialog-pairings]');const dialogTip=document.querySelector('[data-dialog-tip]');const dialogStorage=document.querySelector('[data-dialog-storage]');document.querySelectorAll('[data-product-card]').forEach(card=>{const btn=card.querySelector('button');btn?.addEventListener('click',()=>{if(!dialog)return;if(dialogKicker)dialogKicker.textContent=window.TS_TRANSLATE?window.TS_TRANSLATE(card.dataset.kicker||'Product details'):(card.dataset.kicker||'Product details');if(dialogTitle)dialogTitle.textContent=window.TS_TRANSLATE?window.TS_TRANSLATE(card.dataset.title||'Product details'):(card.dataset.title||'Product details');if(dialogCopy)dialogCopy.textContent=window.TS_TRANSLATE?window.TS_TRANSLATE(card.dataset.detail||card.dataset.description||card.dataset.note||''):(card.dataset.detail||card.dataset.description||card.dataset.note||'');if(dialogPair)dialogPair.textContent=window.TS_TRANSLATE?window.TS_TRANSLATE(card.dataset.best||card.dataset.pair||card.dataset.pairings||'Message LEE for pairing guidance.'):(card.dataset.best||card.dataset.pair||card.dataset.pairings||'Message LEE for pairing guidance.');if(dialogTip)dialogTip.textContent=window.TS_TRANSLATE?window.TS_TRANSLATE(card.dataset.tip||'Use thoughtfully and taste as you go.'):(card.dataset.tip||'Use thoughtfully and taste as you go.');if(dialogStorage)dialogStorage.textContent=window.TS_TRANSLATE?window.TS_TRANSLATE(card.dataset.storage||'Check pack guidance and store carefully.'):(card.dataset.storage||'Check pack guidance and store carefully.');dialog.showModal?dialog.showModal():dialog.setAttribute('open','')})});document.querySelector('[data-dialog-close]')?.addEventListener('click',()=>dialog?.close());dialog?.addEventListener('click',event=>{const rect=dialog.getBoundingClientRect();const outside=event.clientX<rect.left||event.clientX>rect.right||event.clientY<rect.top||event.clientY>rect.bottom;if(outside)dialog.close()});
